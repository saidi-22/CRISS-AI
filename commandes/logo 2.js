...
zokou({ nomCom: "hacker",
    ...
    if (!arg || arg == "") {
        repondre("__Exemple :  " + prefixe + "hacker 𝗖𝗿𝗶𝘀𝘀");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: " \t Logo by 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        const noArgMsg = `__EXEMPLE :  ${prefixe}dragonball 𝗖𝗿𝗶𝘀𝘀`;
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: " \t Logo by 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        if (!arg || arg == '') {
            repondre("__Exemple :  " + prefixe + "naruto 𝗖𝗿𝗶𝘀𝘀");
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t Logo by 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "didong", categorie: "Logo", reaction: "📱" }, async (dest, zk, commandeOptions) => {
    let { arg, repondre, prefixe, ms } = commandeOptions;
   try {
     var titre = "\t logo by 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗";
      var lien = "https://ephoto360.com/tao-anh-che-vui-tu-choi-cuoc-goi-voi-ten-cua-ban-930.html";
        if (!arg || arg == "") {
           repondre(`*exemple :* ${prefixe}didong 𝗖𝗿𝗶𝘀𝘀`)
            return ;
        }
...
zokou({nomCom: "neonlight", categorie: "Logo", reaction: "💡"}, async (dest, zk, commandeOptions) => {
  ...
  try{
    ...
    /*.then((data) => {*/
     zk.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo by 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗' }, { quoted: ms });
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗*"})
         
        // await zk.sendMessage(dest,{image:{url:lienImage},caption:"\t *Logo by 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗*"})
...
zokou({nomCom:"gold",categorie:"Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"gold 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗");return;
      }
...
zokou({nomCom:"arena",categorie:"Logo",reaction:"🥵"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"arena 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗");return;
      }
...
zokou({nomCom:"incandescent",categorie:"Logo",reaction:"😋"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"incandescent 𝗖𝗥𝗜𝗦𝗦-𝗠𝗗");return;
      }
